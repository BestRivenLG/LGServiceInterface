...
public interface PageViewMapper extends BaseMapper<PageView> {
    ...
    @Select("SELECT p.id, p.category_id, (pc.user_id = #{userId} AND p.id = pc.photo_id) AS collect, p.icon, p.ratio, p.title, p.resource_type, p.video_url, p.page_view, p.page_text, p.images, p.en_title FROM photos p JOIN page_view pv ON p.id = pv.photo_id AND pv.user_id = #{userId} LEFT JOIN photos_collect pc ON p.id = pc.photo_id AND pc.user_id = #{userId} ORDER BY pv.update_time DESC")
//    @Select("SELECT p.id, p.category_id, (pc.user_id = #{userId} AND p.id = pc.photo_id) AS collect, p.icon, p.ratio, p.title, p.resource_type, p.video_url, p.page_view, p.page_text, p.images, p.en_title FROM photos p LEFT JOIN photos_collect pc ON p.id = pc.photo_id AND pc.user_id = #{userId} JOIN page_view pv ON p.id = pv.photo_id AND pv.user_id = #{userId} ORDER BY pv.update_time DESC")
